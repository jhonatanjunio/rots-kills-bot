...
export class DeathMonitor {
  ...
  private async processPlayer(player: Player) {
    try {
      ...
      if (!updatedPlayer) {
        // Se não encontrou o jogador pelo nome atual, tenta buscar pelo ID
        const searchResult = await GameAPI.searchPlayerById(player.id);
        
        if (searchResult && searchResult.name !== player.name) {
          // Encontrou o jogador com um nome diferente
          console.log(`🔄 Detectada mudança de nome: ${player.name} -> ${searchResult.name}`);
          logtail.info(`Detectada mudança de nome: ${player.name} -> ${searchResult.name}`);
          ...
          if (channel) {
            const nameChangeAlert = new EmbedBuilder()
              ...
              .addFields(
                { name: 'Nick Anterior', value: player.name, inline: true },
                { name: 'Novo Nick', value: searchResult.name, inline: true }
              )
              .setTimestamp();
          ...
          await Database.updatePlayerName(player.name, searchResult.name);
          return; // Encerra o processamento atual
        }
      }
